  ...
  >
    <Card className="mt-3 total">
      <Card.Body>
        ...
        <Card.Text>
          Total: {totalIncome.toFixed(2)} CHF
        </Card.Text>
      </Card.Body>
    </Card>
  </motion.div>
  ...
  <Row className="grid-row">
    ...
    <Col md={4}>
      <Form.Group>
        ...
        <Form.Control type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Income Amount in CHF" required />
      </Form.Group>
    </Col>
  </Row>
  <Row className="grid-row">
  ...
  {filteredIncomes.map((income) => (
    <ListGroup.Item key={income.id} className="list-group-item">
      <div className="expense-details">
        {`${income.name} - Amount: CHF${income.amount} - Date: ${income.date} - Type: ${income.description} - Category: ${income.category || 'Not specified'} - Status: ${income.status}`}
      </div>
